...
// 强类型枚举引入了类型安全和作用域。在C++11及以后的版本中，引入了强类型枚举（enum
// class）：
enum class file_type { text, image, binary, unknown };
...
enum_sample_file::enum_sample_file(const std::string &name)
    : file_name(name), type(file_type::unknown) {}

std::string enum_sample_file::get_name() const { return this->file_name; }

file_type enum_sample_file::get_file_type() const { return this->type; }
