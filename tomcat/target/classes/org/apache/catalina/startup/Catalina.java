...
// Add RuleSets for nested elements
    ...
    public void load() {
        ...
        // 解析server.xml，实例化一个Server，使Catalina与Server绑定，设置Server的Base和home目录
        getServer().setCatalina(this);
        getServer().setCatalinaHome(Bootstrap.getCatalinaHomeFile());
        getServer().setCatalinaBase(Bootstrap.getCatalinaBaseFile());
        // SystemLogHandler 基于 System.out  System.err设置流对象
        // Stream redirection
        initStreams();
    ...
    public void load(String[] args) {
        // 带参数的load方法处理了参数之后调用无参的load方法，启动的时候没有传参就不会执行到这里
        try {
            if (arguments(args)) {
                load();
            }
    ...
    public void start() {
        ...
        // 正常情况下肯定不为null，万一为null,重新load()初始化一下
        if (getServer() == null) {
            load();
        }
        ...
        //log.info("Tomcat的启动使用的类加载器是：" + this.parentClassLoader.getClass());
    ...
    protected void initDirs() {
        String temp = System.getProperty("java.io.tmpdir"); //临时的工作目录，实际上是与当前的JVM线程的环境相绑定的
