    ...
    fun zfbDonate(context: Context) {
        try {
            QMUIDialog.MessageDialogBuilder(context)
                ...
                .addAction("支付宝") { dialog, index ->
                    dialog.dismiss()
                    ToastUtils.showLong(Utils.getApp().getString(R.string.app_name) + " 因为有你的支持而能够不断更新、完善，非常感谢支持！")
                    val intent = Intent(Intent.ACTION_VIEW)
                    intent.data =
                        Uri.parse("alipays://platformapi/startapp?saId=10000007&clientVersion=3.7.0.0718&qrcode=https%3A%2F%2Fqr.alipay.com%2Ffkx15436xnv3mzpuufhvn52%3F_s%3Dweb-other")
